...
class MyCallback(keras.callbacks.Callback):
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        K.set_value(self.model.optimizer.lr, 0.01 / ((1. + 10. * float(batch) / K.get_value(self.model.total_steps)) ** 0.75))

        # Schedule for domain adaptation parameter
        # for layer in self.model.layers:
        #     if isinstance(layer, GradientReversal):
        #         K.set_value(layer.hp_lambda, 2./(1.+ np.exp(-10.*float(batch)/K.get_value(self.model.total_steps)))-1.)
        #         break
